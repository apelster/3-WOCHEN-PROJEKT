...
import { registerUser, authenticateUser, confirmUser } from './auth.mjs';dotenv.config();const app = express();
const PORT = process.env.PORT || 3002;const corsOptions = {
  origin: '*', // Erlauben Sie alle Ursprünge
  optionsSuccessStatus: 200,
  methods: "GET,HEAD,PUT,PATCH,POST,DELETE",
  allowedHeaders: ['Content-Type', 'Authorization']
};app.use(cors(corsOptions));
app.use(bodyParser.json());// Registrierung
...
app.post('/confirm', (req, res) => {
  ...
  console.log('Empfangene Daten:', { email, code }); // Log received data  confirmUser(email, code, (err, result) => {
    if (err) {
...
});app.listen(PORT, '0.0.0.0', () => { // Bind the server to all network interfaces
